...
class SimulationRecovery:
    ...
    def run_optimization(self, config, checkpoint_dir=None):
        ...
        try:
            params_before, params_after_optim, losses = self._optimize(
                self.Y_t, self.X_t, self.Z_t, config
            )
    ...
    def recovery_for_simulation(
        ...
        best_params, best_Y_predicted, best_metrics, best_final_loss = (
            self.run_optimization(best_config)
        )
        ...
        utils.calculate_statistics(
            Gs, etas, zetas, gammas, 0
        )  # Assuming best_run_number = 0

        return best_Y_predicted

    def _optimize(
        self, Y_t: torch.Tensor, X_t: torch.Tensor, Z_t: torch.Tensor, config
    ) -> tuple:
        optimizer = optim.SGD(
...
def simulation_recovery(X_t, Z_t, Y_t):
    ...
    for G in G_list:
        ...
        analysis = tune.run(
            lambda config: SimulationRecovery(X_t, Z_t, simulated_Y).run_optimization(
                config
            ),
            config={
                "learning_rate": tune.loguniform(1e-4, 1e-1),
            ...
            scheduler=ASHAScheduler(
                metric="loss", mode="min", max_t=config["modelTraining"]["epoch"]
            ),
        )
