...
class DynamicLinearModel:
    def __init__(self, model_path=None):
        ...
        self.eta = np.random.normal(size=7)
        self.zeta = np.random.normal(size=2)
    ...
    def dlm_model(self, X_t, Z_t):
        ...
        for t in range(T):
            ...
            predicted_Y[t] = (
                config["modelTraining"]["addWeight"] * theta_t[t] + np.dot(X_t[t], self.eta) + np.dot(Z_t[t], self.zeta / 2)
            )
