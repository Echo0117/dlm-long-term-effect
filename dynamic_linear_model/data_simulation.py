...
class SimulationRecovery():
    ...
    def recovery_for_simulation(self):
        ...
        Y_predicted_list = []       
        for _ in range(config["simulationRecovery"]["independentRun"]):
            print("independentRun: ", _)
            initial_params = self._initialize_parameters()
            try:
                params = self._optimize(self.Y_t, self.X_t, self.Z_t, initial_params)
            except UnboundLocalError as e:
    ...
    def _plot_g(self, g_list):
        """
        Plot the G values for each run.
        """
        plt.figure(figsize=(14, 6))
        plt.plot(range(config["simulationRecovery"]["independentRun"]), g_list, 'b-', label='G')
        plt.title('G during Recovery')
        plt.xlabel('Epoch')
        plt.ylabel('G')
        plt.legend()
        plt.tight_layout()
